...
function showAllCity() {
	...
	tHtml = '<div class="com_hotresults" id="thetable" style="width:15rem">'
			+ '<div style="width:100%;">'
			+ '<div class="ac_title">'
			+ '<span>'
			+ '拼音支持首字母输入'
			...
			+ '<ul class="popcitylist" style="min-height: 8rem;" method="hotData" id="ul_list1">';
	...
	for ( var a = 2; a <= 6; a++) {
		...
		if (totelLength > ulPageSize) {
			...
			var pagecount = Math.ceil((totelLength + 1) / ulPageSize);
			if (pagecount > 1) {
				tHtml += '<ul  class="popcitylist" style="min-height: 8rem;display:none;" id="ul_list'
						+ a + '">';
	...
	$('#thetable').delegate('li','click', function() {
	...
	$('#thetable').delegate('.ac_even','mouseover', function() {
		city_shiftSelectInLi(this);
	}).delegate('.ac_even','click', function() {
...
function SetISPos(obj) {
	...
	console.log(obj.position().top +"-------------"+ obj.height())
	$("#form_cities2").css("top", obj.position().top + obj.height() -45);
}
